                ...
                if(data.containsKey("phoneState")){
                    ...
                    switch(state){
                        case TelephonyManager.CALL_STATE_IDLE:
                            notifyPrivacyCallState(TelephonyManager.CALL_STATE_IDLE, 
                                null, packageName);
                            return;
                        case TelephonyManager.CALL_STATE_OFFHOOK:
                            notifyPrivacyCallState(TelephonyManager.CALL_STATE_OFFHOOK,
                                null, packageName);
                            return;
                        case TelephonyManager.CALL_STATE_RINGING:
                            notifyPrivacyCallState(TelephonyManager.CALL_STATE_RINGING, 
                                "12345", packageName);
                            return;
                        default:
                            return;
    ...
    private boolean isPackageAllowed(int PERMISSION, String packageName){
        ...
        switch(PERMISSION){
            case PERMISSION_CELL_LOCATION:
                ...
                //  if(((settings.getLocationNetworkSetting() != PrivacySettings.REAL) || 
                //      (settings.getLocationGpsSetting() != PrivacySettings.REAL)))
                    return false;
                } else { 
                    return true;
                }
                ...
                if ((r.events & PhoneStateListener.LISTEN_CELL_INFO) != 0) {
                    try {
                        if(!isPackageAllowed(PERMISSION_CELL_INFO,r.pkgForDebug)){
                            ...
                            CellIdentityGsm fakeCellIdentity = 
                                new CellIdentityGsm(11,11,549,525,2);
                            fakeCellInfo.setCellIdentity(fakeCellIdentity);
                            //  r.callback.onCellInfoChanged
                            //  (new CellInfoGsm(CellInfo.TIMESTAMP_TYPE_UNKNOWN,
                            //  System.currentTimeMillis(),System.currentTimeMillis(),
                            //  true,new SignalStrength(),new CellIdentityGsm
